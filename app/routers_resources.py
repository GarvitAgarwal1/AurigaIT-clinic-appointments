...
from .services import advance_clock, book_appointment, cancel_appointment, get_current_time, reschedule_appointment
...
def doctors(page_number: int = Query(1, alias="page", ge=1), page_size: int = Query(20, alias="size", ge=1, le=100), sort: str = "name", db: Session = Depends(get_db)):
    ...
    items = page(db.query(Doctor).order_by(asc(allowed.get(sort, Doctor.name))), page_number, page_size)
...
def create_doctor(data: DoctorIn, db: Session = Depends(get_db)):
    doctor = Doctor(**data.model_dump()); db.add(doctor); db.commit(); db.refresh(doctor)
    return doctor
...
def delete_doctor(doctor_id: int, db: Session = Depends(get_db)):
    doctor = db.get(Doctor, doctor_id)
    if not doctor: raise HTTPException(404, "Doctor not found")
    if db.query(Appointment).filter(Appointment.doctor_id == doctor_id).first(): raise HTTPException(409, "Doctor has appointments")
...
def create_patient(data: PatientIn, db: Session = Depends(get_db)):
    patient = Patient(**data.model_dump()); db.add(patient); db.commit(); db.refresh(patient); return patient
...
def delete_patient(patient_id: int, db: Session = Depends(get_db)):
    patient = db.get(Patient, patient_id)
    if not patient: raise HTTPException(404, "Patient not found")
    if db.query(Appointment).filter(Appointment.patient_id == patient_id).first(): raise HTTPException(409, "Patient has appointments")
...
def doctor_day(doctor_id: int, day: date, page_number: int = Query(1, alias="page", ge=1), page_size: int = Query(100, alias="size", ge=1, le=200), db: Session = Depends(get_db)):
    ...
    query = db.query(Appointment).filter(Appointment.doctor_id == doctor_id, Appointment.starts_at >= start, Appointment.starts_at <= end).order_by(asc(Appointment.starts_at))
    return page(query, page_number, page_size)


@router.post("/appointments", response_model=AppointmentOut, status_code=201)
def create_appointment(data: AppointmentIn, db: Session = Depends(get_db)):
    if not db.get(Doctor, data.doctor_id) or not db.get(Patient, data.patient_id): raise HTTPException(404, "Doctor or patient not found")
...
def reschedule(appointment_id: int, data: RescheduleIn, db: Session = Depends(get_db)):
    appointment = db.get(Appointment, appointment_id)
    if not appointment: raise HTTPException(404, "Appointment not found")
    return reschedule_appointment(db, appointment, data.starts_at, data.ends_at)
...
@router.post("/clock")
def set_clock(data: ClockIn, db: Session = Depends(get_db)):
...
def outbox(db: Session = Depends(get_db)):
    notifications = db.query(OutboxNotification).order_by(OutboxNotification.id).all()
    return [{
        "id": item.id,
